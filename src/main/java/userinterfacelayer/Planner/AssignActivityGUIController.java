...
public class AssignActivityGUIController implements Initializable {
    ...
    private ArrayList<MaintenanceActivity> activityList = null;
    ...
    private MaintenanceActivity findObject() {
        ...
        activityList = planner.viewActivities();
    ...
    private void btnAssignSel_OnAction(ActionEvent event) {
        if (tableView.getSelectionModel().getSelectedItem() == null) {
            ...
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getClassLoader().getResource("CheckAssignedActivityGUI.fxml"));
    ...
    private void initializeTable(int week) { //get the activity with the same week and build the new object used to load the table
        ...
        activityList = planner.getActivitiesByWeek(week);
        for (int i = 0; i < activityList.size(); i++) {
